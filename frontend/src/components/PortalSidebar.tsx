...
interface MenuItem { title: string; icon: LucideIcon; path: string; subItems?: { title: string; path: string }[]; }
interface PortalSidebarProps { menuItems: MenuItem[]; portalName: string; logoutPath?: string; }
...
export function PortalSidebar({ menuItems, portalName, logoutPath = "/login" }: PortalSidebarProps) {
  ...
  return (
    <>
      <motion.aside initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.5, ease: "easeOut" }}
        ...
        <nav className="sidebar-nav-scroll flex-1 space-y-1 px-3 overflow-y-auto">
          {menuItems.map((item, index) => {
            ...
            return (
              <div key={item.title}>
                <motion.button initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: index * 0.05, duration: 0.3 }}
                  onClick={() => {
                    if (item.subItems) {
                  ...
                  className={`group flex w-full items-center justify-between rounded-xl px-4 py-3 text-sm font-medium transition-all duration-200 ${isActive ? "bg-sidebar-primary/20 text-sidebar-foreground shadow-glow backdrop-blur-sm" : "text-sidebar-foreground/70 hover:bg-sidebar-primary/10 hover:text-sidebar-foreground"}`}>
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                      <div className="mt-1 flex flex-col gap-1 pl-11 pr-2">
                        {item.subItems.map(subItem => (
                          <button key={subItem.path} onClick={() => navigate(subItem.path)}
                            className={`flex w-full items-center rounded-lg px-3 py-2 text-sm transition-all duration-200 ${location.pathname === subItem.path ? "bg-sidebar-primary/20 text-sidebar-foreground font-semibold" : "text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-sidebar-primary/10"}`}>
                            {subItem.title}
                          </button>
                        ))}
                      </div>
        ...
        <div className="px-3 pb-4">
          <button onClick={() => setShowLogout(true)} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-sidebar-foreground/70 transition-all hover:bg-sidebar-primary/10 hover:text-sidebar-foreground">
      ...
      <LogoutModal open={showLogout} onClose={() => setShowLogout(false)} onConfirm={() => navigate(logoutPath)} />
    </>
  );
}
