...
import { StaffSalaryPaymentDto, formatPayrollCurrency, payrollMonthName } from "@/types/payroll";

interface StaffPaymentDetailsDialogProps {
  payment: StaffSalaryPaymentDto | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onDeleteLine?: (paymentId: string, lineId: string) => void;
}

export function StaffPaymentDetailsDialog({ payment, open, onOpenChange, onDeleteLine }: StaffPaymentDetailsDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {payment?.staffName} - {payment && payrollMonthName(payment.month)} {payment?.year}. Net:{" "}
